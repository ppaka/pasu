...
class Note:
    ...
    def set_note(self, note: osureader.objects.HitObject, key_index):
        self.start_time = note.time
        self.note_type = 0
        self.key_index = key_index

    def set_hold_note(self, note: osureader.objects.HitObject, key_index):
        self.start_time = note.time
        self.note_type = 1
        self.end_time = int(note.extras.split(":")[0])
        self.hold_length = self.end_time - self.start_time
        self.key_index = key_index
...
def get_audio_file_name(location) -> str:
...
def load_notes(location) -> list:
    ...
    for hit_object in hit_objects:
        note = Note()
        lane_index = hit_object.point.x * 4 / 512
        lane_index = int(lane_index)
        if lane_index < 0:
            lane_index = 0
        elif lane_index > 3:
            lane_index = 3
        if hit_object.type == osureader.objects.HitObjectType.MANIA_HOLD:
            note.set_hold_note(hit_object, lane_index)
        else:
            note.set_note(hit_object, lane_index)
        notes.append(note)
    return notes
...
pg.mixer.music.load(beatmapDir + "/" + beatmapSongFileName)  # 음악 리소스 불러오기
pg.mixer.music.set_volume(0.05)

clock = pg.time.Clock()
game_time -= int(offset_by_audio_buffer)
